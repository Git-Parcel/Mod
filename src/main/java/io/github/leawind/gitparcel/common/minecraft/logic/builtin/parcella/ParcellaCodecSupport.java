...
final class ParcellaCodecSupport {
  ...
  private ParcellaCodecSupport() {}

  static void reconcileDataRoot(Path dataRoot, Set<String> componentDirectories)
      throws IOException {
    if (!Files.isDirectory(dataRoot, LinkOption.NOFOLLOW_LINKS)) return;
    try (var entries = Files.newDirectoryStream(dataRoot)) {
      for (Path entry : entries) {
        if (!componentDirectories.contains(entry.getFileName().toString())) {
  ...
  static void validateDataRoot(Path dataRoot, Set<String> componentDirectories)
    ...
    if (!Files.isDirectory(dataRoot, LinkOption.NOFOLLOW_LINKS)) {
      throw new ParcelException.CorruptedParcelException(
          "Parcella data directory not found: " + dataRoot);
    }
    try (var entries = Files.newDirectoryStream(dataRoot)) {
      for (Path entry : entries) {
        String name = entry.getFileName().toString();
        if (!componentDirectories.contains(name)
            || !Files.isDirectory(entry, LinkOption.NOFOLLOW_LINKS)) {
          throw new ParcelException.CorruptedParcelException(
              "Unexpected Parcella data entry: " + entry);
        }
      }
    }
  }
  ...
  static void readRecordDirectory(Path directory, String suffix, FileConsumer consumer)
    ...
    if (!Files.isDirectory(directory, LinkOption.NOFOLLOW_LINKS)) {
      throw new ParcelException.CorruptedParcelException(
          "Expected component directory: " + directory);
    }
    ...
    try (var paths = Files.list(directory)) {
      ...
      while (iterator.hasNext()) {
        ...
        if (!Files.isRegularFile(path, LinkOption.NOFOLLOW_LINKS)
          ...
          throw new ParcelException.CorruptedParcelException(
              "Unexpected component entry: " + path);
        }
  ...
  static void validateComponentFiles(Path directory, Set<Path> expectedFiles)
      throws IOException, ParcelException.CorruptedParcelException {
    Set<Path> normalized =
    ...
    if (unexpected[0] != null) {
      throw new ParcelException.CorruptedParcelException(
          "Unexpected component entry: " + unexpected[0]);
    }
  }
  ...
  static final class ManagedOutput {
    ...
    Path file(Path path) throws IOException {
      ...
      if (!normalized.startsWith(root) || normalized.equals(root)) {
        throw new IOException("Component path escapes its directory: " + path);
      }
      if (!liveFiles.add(normalized)) {
        throw new IOException("Component file written more than once: " + path);
      }
          ...
          new SimpleFileVisitor<>() {
            ...
            public FileVisitResult visitFile(Path file, BasicFileAttributes attributes)
              ...
              if (!attributes.isRegularFile()) {
                throw new IOException("Unsupported component entry: " + file);
              }
