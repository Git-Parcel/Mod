...
- gametest 的世界在 `<节点>/build/run/gameTest/world`，loom 的 `clearRunDirectory` 不覆盖它，本地会跨运行累积已保存数据（如地图 id 计数），于是"只在全新世界里暴露"的问题在本地静默通过，而 CI 每次都是全新检出。`build.gradle.kts` 现已在 `runGameTest` 执行前清掉世界目录，使两侧条件一致；判断某个 gametest 失败是否真实，先确认它是在全新世界下复现的。
