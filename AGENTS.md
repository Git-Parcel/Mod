...
- 1.20.1 的地图 id **0 是合法 id**：`MapIndex.getFreeAuxValueForMap()` 取 `usedAuxIds`（默认值 −1）加 1，全新世界分配的第一张地图就是 0。判定"物品有没有地图 id"必须测键的存在性（`NbtReads.getIntOrNull`，等价于原版 `contains("map", 99)`），用 `> 0` 之类的数值范围判断会把该地图静默丢掉，使它的美术数据不随包裹迁移。
- gametest 的世界在 `<节点>/build/run/gameTest/world`，本地会跨运行累积已保存数据（如地图 id 计数），于是"只在全新世界里暴露"的问题在本地静默通过，而 CI 每次都是全新检出。loom 自带的 `clearRunDirectory` 本应注册 `deleteGameTestRunDir` 任务在 `runGameTest` 前清理运行目录，但 ModStitch 的 `fabric-loom-remap` 平台绕过了这条接线（任务从未注册，`runGameTest` 显示绑定 'null' run configuration），该开关在本构建中是死配置、已移除。`build.gradle.kts` 现于 `runGameTest` 执行前自行清掉世界目录，使两侧条件一致；判断某个 gametest 失败是否真实，先确认它是在全新世界下复现的。
- Windows 上提交的 `gradlew` 在 git 索引里是 `100644`（NTFS 不跟踪可执行位，`core.filemode=false`），ubuntu runner 上 `./gradlew` 直接 `Permission denied`（exit 126），CI 从头到尾没跑起来过。wrapper 的索引模式必须保持 `100755`（`git update-index --chmod=+x gradlew`）；`core.filemode=false` 时 git 沿用索引中的模式，Windows 上后续 `git add` 不会把它退回 644。
