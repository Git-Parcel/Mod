...
- 不检测、不处理 parcel 空间重叠；若将来需要禁止重叠，在创建与调整边界时增加校验即可。
...
按稳定游标使用的顺序显示快照节点。每项包含快照 ID、父 ID、创建时间、作者和名称；`*` 表示当前基准。命令默认显示 20 项，最大 100 项。仓库读取由统一后台操作执行，完成后再输出结果。理由：大树的读取耗时不可控，同步执行会卡住命令线程，统一走操作管理器还能复用进度与限流。
...
Git 自己负责对象内容寻址、相同 blob/tree 去重、zlib 压缩和后续 pack。模组不再实现第二套 blob、分块或摘要数据库。跨归档仓库不共享对象库，以换取故障、并发和删除边界的完全隔离；如将来确有必要，可以评估 Git alternates，但它不属于初始设计。
...
通过 `@AutoService` 或手写 `META-INF/services/io.github.leawind.gitparcel.common.api.extension.GitParcelExtension` 资源文件完成发现。所有注册在启动时一次性完成并冻结；运行期不可再注册。接口生命周期在代码中用 `@ApiStatus` 注解注明（如 `@ApiStatus.Experimental`、`@ApiStatus.Internal`）；API 文档将同时面向人类与 AI 读者发布。本模组可以提供 GameTest 相关工具，第三方扩展的测试由编写它的项目自行负责。
...
- 坐标类空间边是 parcel 系引用，随 parcel 平移/镜像/旋转。目标语义是内指字段随 parcel 变换、外指字段保持恒等；当前实现对两者都做换算，范围判定见 SEMANTICS.md 第 9 节缺口登记。真正“世界外部”的语义应改用附件。
...
- 处理器运行时异常的隔离与容错策略暂未设计：当前扩展异常直接使所属操作失败，待核心功能完备后再定。
...
- 服务端权威和客户端只读镜像。
- `Parcel`（世界侧）与 `ParcelMeta`（内容侧）的现有划分与 parcel/归档二分同构，可作为演化的起点。
- `ParcelBlockTransform` 适配器把版本敏感的方块状态 API 挡在可序列化变换模型之外，语义符合 SEMANTICS.md 规则 3.4。
...
- 捕获钩子向处理器暴露活的 `Entity`/`BlockEntity` 对象而非中立记录，兼容模组被迫产生对目标模组的类路径依赖。

因此，当前代码适合作为内容存储、世界读写、Git 交换和安全规则的原型，但不适合继续把 Git 工作树作为内部快照领域的中心。下一步应先替换领域用例和存储边界，再继续扩充命令。

## 实施顺序

建议按以下顺序开发：

- 定义不向普通玩家暴露 Git 细节的 `SnapshotId`、`SnapshotNode`、树查询、保存/恢复请求和结构化结果。
...
理由：顺序遵循"先替换被证明错误的存储中心，再扩展表层能力"——领域用例与存储边界不稳时，命令上的任何投入都会在重构中重做。
