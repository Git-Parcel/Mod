...
内容应用采用替换语义：方块区域整体覆盖；实体先按捕获谓词（AABB 相交、非玩家、非乘客的根实体，乘客树随根）清除区域内旧实体，再批次生成快照实体；玩家永不清除。恢复完成后区域内的实体集合与快照实体集同构（SEMANTICS.md 规则 6.2、不变式 6.2）。方块放置尽量抑制副作用（邻居更新、进度与成就触发、音效粒子等），避免结果依赖放置顺序或中途状态引发连锁反应；光照由引擎在放置后统一重算。理由：方块已是覆盖语义，实体若只追加，重复恢复同一快照会让实体翻倍；清除谓词与捕获谓词是同一函数，往返不变性才能对实体集合成立。恢复是批量重建而非逐块交互，逐块触发更新会引爆恢复中的红石装置。
