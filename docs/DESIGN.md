...
内置处理器覆盖原版的蜂巢/蜂箱 `flower_pos`、拴绳 `leash`（UUID 与坐标两种形态）等语境依赖字段；原版字段的处理器随本模组按 Minecraft 版本维护（见“版本接缝与双轴版本”）。
...
- 物品内嵌坐标（如探险家指南针的 `LodestonePos`）暂无内置处理器；处理器直接操作 NBT，扩展可自行实现。
