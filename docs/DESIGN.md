...
- 稳定 UUID。
- 所属维度和空间范围。
- 锚点与当前放置变换。锚点是持久参考点，不依附于任何边界角点，其存储语义见“锚点参照系与子区对齐”。
- 名称、说明和视觉配置。
- parcel 权限配置。
- 新快照默认使用的格式及配置。
...
parcel 的锚点是持久参考点，不依附于任何边界角点。调整边界不改变锚点的绝对坐标；整体移动属于放置变换的变化，锚点随之移动。快照中的定位字段与已覆盖的内指空间边一律为锚点相对坐标，未被处理器覆盖的载荷字段按公理 2 保持世界原值，其迁移后的大 diff 是语义诚实的代价。文件与目录命名只用锚点相对的格点索引和稳定标识（UUID、内容摘要），不使用世界坐标。
...
- 记录处理器（`ParcelRecordProcessor`）：无状态、服务器线程，在捕获/恢复时逐记录变换中立记录（记录种类、类型 ID、NBT 载荷与定位字段），可声明 `runAfter`/`runBefore` 顺序。处理器不自行组合镜像与旋转，只请求 `ParcelSpace` 施加完整变换（SEMANTICS.md 规则 3.3）；身份边在批次句柄翻译阶段重写（SEMANTICS.md 规则 4.1）；瞬态与派生载荷的消除与相对化同样是处理器职责（SEMANTICS.md 定义 2.5、规则 2.2、规则 2.3）。处理器私有的版本化侧数据通过 `SemanticData` 挂在记录上。
